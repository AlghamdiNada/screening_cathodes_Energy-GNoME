...
if not os.path.exists(f'/leonardo_scratch/large/userexternal/nalghamd/mc_mace/GNoME-screening/correct_phonon_run/Na/6a36329b61/relaxed.cif'):
    atoms = read(f'/leonardo_scratch/large/userexternal/nalghamd/mc_mace/GNoME-screening/correct_phonon_run/Na/6a36329b61/6a36329b61.CIF')
    atoms.calc = calculator
    print('Starting relaxation')
    dyn = FIRE2(atoms)
    dyn.run(fmax=1e-4)
    write(f'/leonardo_scratch/large/userexternal/nalghamd/mc_mace/GNoME-screening/correct_phonon_run/Na/6a36329b61/relaxed.cif', atoms)
else:
    atoms = read(f'/leonardo_scratch/large/userexternal/nalghamd/mc_mace/GNoME-screening/correct_phonon_run/Na/6a36329b61/relaxed.cif')
...
try:
    ...
    with open(f'/leonardo_scratch/large/userexternal/nalghamd/mc_mace/GNoME-screening/correct_phonon_run/Na/6a36329b61/phonon_bandstructure.json', 'w') as f:
        bs.write(f)
    ...
    plt.savefig(f'/leonardo_scratch/large/userexternal/nalghamd/mc_mace/GNoME-screening/correct_phonon_run/Na/6a36329b61/bands.pdf')
    print('Phonon band structure calculation completed successfully')

except Exception as e: print(e)

# ---- load bandstructure (ASE JSON) ----
if os.path.exists(f'/leonardo_scratch/large/userexternal/nalghamd/mc_mace/GNoME-screening/correct_phonon_run/Na/6a36329b61/phonon_bandstructure.json'):
    with open(f'/leonardo_scratch/large/userexternal/nalghamd/mc_mace/GNoME-screening/correct_phonon_run/Na/6a36329b61/phonon_bandstructure.json', 'r') as f:
        bs = BandStructure.read(f) 
    n_negative_bands = len(np.where(bs.energies < -0.004)[0])
    exists_imaginary = n_negative_bands > 0
else:
    print(f'No phonon_bandstructure.json found in /leonardo_scratch/large/userexternal/nalghamd/mc_mace/GNoME-screening/correct_phonon_run/Na/6a36329b61/phonon_bandstructure.json, skipping...')
...
df.to_csv(f'/leonardo_scratch/large/userexternal/nalghamd/mc_mace/GNoME-screening/correct_phonon_run/Na/6a36329b61/info.csv', index=False)
